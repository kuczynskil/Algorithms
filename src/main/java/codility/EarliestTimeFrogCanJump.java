...
public class EarliestTimeFrogCanJump {
...
// side of the river. The frog can cross only when leaves appear at every position across the
// river from 1 to X (that is, we want to find the earliest moment when all the positions from
// 1 to X are covered by leaves). You may assume that the speed of the current in the river is
...
//each element of array A is an integer within the range [1..X].
    ...
    public int solution(int X, int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            ...
            if (sum == sumOfConsecutive) return i;
        }
        return -1;
    }

    public static void main(String[] args) {
        int i = new EarliestTimeFrogCanJump().solution(5, new int[] {1,3,1,4,2,3,5,4});
