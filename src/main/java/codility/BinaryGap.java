...
// length 2. The number 529 has binary representation 1000010001 and contains two binary gaps: one of length 4 and one of length 3. The number 20 has binary representation 10100 and contains one binary gap of length 1. The number 15 has binary representation 1111 and has no binary gaps. The number 32 has binary representation 100000 and has no binary gaps.
//
//Write a function:
//
...
//N is an integer within the range [1..2,147,483,647].
    ...
    public int solution(int N) {
        ...
        for (int i = 0; i < binary.length(); i++) {
            if (binary.charAt(i) == '1') indexesOfOnes.add(i);
        }
        int longestGap = 0;
        for (int i = 0; i < indexesOfOnes.size() - 1; i++) {
            int gap = indexesOfOnes.get(i+1) - indexesOfOnes.get(i) - 1;
