...
//    or more characters. The 1st character of a code is a capital letter which defines the book category.
//
...
//    If L or M are empty return string is "" (Clojure and Racket should return an empty array/list instead).
    ...
    public static String stockSummary(String[] lstOfArt, String[] lstOf1stLetter) {
        ...
        Map<Character, Integer> quantitiesCategories = new HashMap<>();
        for (String s : lstOf1stLetter) {
            quantitiesCategories.put(s.toCharArray()[0], 0);
        }
        for (String s : lstOfArt) {
            String[] tmp = s.split(" ");
            if (quantitiesCategories.containsKey(tmp[0].charAt(0))) {
                quantitiesCategories.put(tmp[0].charAt(0),
                        quantitiesCategories.get(tmp[0].charAt(0)) + Integer.parseInt(tmp[1]));
            }
        }
        List<String> result = new ArrayList<>();
        quantitiesCategories.forEach((key, value) -> result.add("(" + key + " : " + value + ")"));
